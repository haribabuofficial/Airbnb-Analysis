...
def datafr():
    df = pd.read_csv(r'F:\hari babu\GUVI\Assignments\Airbnb\Airbnb.csv')
    return df
...
if select == 'Home':
    image1= Image.open('F:\hari babu\GUVI\Assignments\Airbnb\Airbnb1.jpg')
    st.image(image1)
    st.header("About Airbnb")
    st. write("")
    st.write('''****Airbnb is an online marketplace that connects people who want to rent out
                  their property with people who are looking for accommodations, typically for 
                  short stays. Airbnb offers hosts a relatively easy way to earn some income from 
                  their property.Guests often find that Airbnb rentals are cheaper and homier than 
                  hotels.***''')
    st.write("")
    st.write('''***Airbnb Inc (Airbb) operates an online platform for hospitality services.
                The company provides a mobile application (app) that enables users to list, discover, and book 
                unique accommodations across the world.
                The app allows hosts to list their properties for lease, and enables guests to rent or lease on 
                a short-term basis, which includes vacation rentals, apartment rentals, homestays, castles, tree 
                houses and hotel rooms. The company has presence in China, India, Japan, Australia, Canada, Austria, 
                Germany, Switzerland, Belgium, Denmark, France, Italy, Norway, Portugal, Russia, Spain, Sweden, the UK, and others.
                Airbb is headquartered in San Francisco, California, the US.***''')
...
if select == 'Data Exploration':
    ...
    with tab2:
        ...
        def datafr():
            df_a = pd.read_csv('F:\hari babu\GUVI\Assignments\Airbnb\Airbnb.csv')
            return df_a
    ...
    with tab3:
        ...
        def datafr():
            df= pd.read_csv('F:\hari babu\GUVI\Assignments\Airbnb\Airbnb.csv')
            return df
...
if select == 'About':
    ...
    st.write('''***Gather data from Airbb's public API or other available sources.
